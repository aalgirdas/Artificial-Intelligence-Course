...
def show_map(graph_data, node_colors=None):
    ...
    for edge in G.edges():
        node1, node2 = edge
        if node_colors[node1] == 'green' and node_colors[node2] == 'green':
            edge_colors.append('Green')
            edge_widths.append(2)
        else:
            edge_colors.append('k')
            edge_widths.append(0.3)
    nx.draw(G, pos={k: node_positions[k] for k in G.nodes()}, node_color=[node_colors[node] for node in G.nodes()] , linewidths=0.3,  edge_color=edge_colors, width=edge_widths, node_size=85)

    #nx.draw(G, pos={k: node_positions[k] for k in G.nodes()}, node_color=[node_colors[node] for node in G.nodes()], linewidths=0.1, edgecolors='k', node_size=85)

    # draw labels for nodes
    node_label_handles = nx.draw_networkx_labels(G, pos=node_label_pos, font_size=10)
    top_10_nodes = list(node_colors.keys())[:10]
    for label in node_label_handles.values():
        if label.get_text() in top_10_nodes:
    ...
    gray_circle = lines.Line2D([], [], color="gray", marker='o', markersize=10, markerfacecolor="gray")
    green_circle = lines.Line2D([], [], color="green", marker='o', markersize=10, markerfacecolor="green")
    plt.legend((white_circle, orange_circle, red_circle, gray_circle, green_circle),
               ('Un-explored', 'Frontier', 'Currently Exploring', 'Explored', 'Final Solution'),
               numpoints=1, prop={'size': 10}, loc=(.84, .85))
